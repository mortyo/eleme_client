...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      //获取category分类左侧数据
      let res_foodCategory = await foodCategory(this.latitude, this.longitude);
      this.category = res_foodCategory.data;
      ...
      //获取筛选列表的配送方式
      let res_foodDelivery = await foodDelivery(this.latitude, this.longitude);
      this.Delivery = res_foodDelivery.data;

      //获取筛选列表的商铺活动
      let res_Activity = await foodActivity(this.latitude, this.longitude);
      this.Activity = res_Activity.data;
