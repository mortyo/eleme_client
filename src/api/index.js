...
export const shopDetails = (shopid, latitude, longitude) =>
  axios.get("/shopping/restaurant/" + shopid, {
    params: {
      latitude,
      longitude:
        longitude +
...
export const getRatingList = (shopid, offset, tag_name = "") =>
  axios.get("/ugc/v2/restaurants/" + shopid + "/ratings", {
    params: {
      has_content: true,
      offset,
      limit: 10,
...
export const ratingScores = shopid =>
  axios.get("/ugc/v2/restaurants/" + shopid + "/ratings/scores");
...
export const ratingTags = shopid =>
  axios.get("/ugc/v2/restaurants/" + shopid + "/ratings/tags");
...
export const postAddAddress = (
  ...
  axios.post("/v1/users/" + userId + "/addresses", {
    address,
    address_detail,
    geohash,
    name,
...
export const rePostVerify = (cart_id, sig, type) =>
  axios.post("/v1/carts/" + cart_id + "/verify_code", {
    sig,
    type
  });
...
export const payRequest = (merchantOrderNo, userId) =>
  axios.get("/payapi/payment/queryOrder", {
    params: {
      merchantId: 5,
      merchantOrderNo,
      source: "MOBILE_WAP",
...
export const vipCart = (id, number, password) =>
  axios.post("/member/v1/users/" + id + "/delivery_card/physical_card/bind", {
    number,
    password
  });
...
export const getHongbaoNum = id =>
  axios.get("/promotion/v2/users/" + id + "/hongbaos?limit=20&offset=0");
...
export const getExpired = id =>
  axios.get(
    "/promotion/v2/users/" + id + "/expired_hongbaos?limit=20&offset=0"
  );
...
export const exChangeHongbao = (id, exchange_code, captcha_code) =>
  axios.post("/v1/users/" + id + "/hongbao/exchange", {
    exchange_code,
    captcha_code
  });
...
export const getOrderList = (user_id, offset) =>
  axios.get("/bos/v2/users/" + user_id + "/orders", {
    params: {
      limit: 10,
      offset: offset
    }
...
export const getOrderDetail = (user_id, orderid) =>
  axios.get("/bos/v1/users/" + user_id + "/orders/" + orderid + "/snapshot");
...
export const deleteAddress = (userid, addressid) =>
  axios.get("/v1/users/" + userid + "/addresses/" + addressid, {}, "DELETE");
